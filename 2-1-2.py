...
class InputConnect:
    ...
    def print_vacancies_info(self, vacancies: list, image_name: str):
        self.fill_vacancies_info(vacancies)
        for key, value in self.vacancies_info.items():
            print(f"{key}: {value}")
        rep = Report(image_name, self.vacancies_info, self.job_name)
        rep.generate_image()
...
if os.path.getsize(csv_file.name) == 0:
    ...
    csv_file.print_vacancies_info(data.vacancies_objects, 'graph.png')
